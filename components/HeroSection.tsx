  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-12 sm:bottom-16 md:bottom-20 left-0 right-0 z-10 flex flex-col items-center text-center px-4">
        ...
        <p 
          className={`mt-2 text-xl sm:text-2xl md:text-3xl font-medium text-sky-600/90 uppercase tracking-wider transition-all-500-ease-out ${isRoleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
        >
          {USER_ROLE}
        </p>
        <TypingText
          ...
          className="mt-3 text-sm sm:text-base md:text-lg font-normal text-sky-500/80 tracking-wide min-h-[1.5em]"
          style={{ textShadow: '0 0 3px rgba(255,255,255,0.3)'}}
        />
      </div>

      {/* Small HUD text elements */}
      <div className={`absolute top-8 left-8 z-20 text-xs font-mono text-sky-500/70 ${areHudElementsVisible ? 'opacity-75' : 'opacity-0'}`}>
      ...
      <div className={`absolute bottom-8 right-8 z-20 text-xs font-mono text-sky-500/70 text-right ${areHudElementsVisible ? 'opacity-75' : 'opacity-0'}`}>
